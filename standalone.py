...
def get_run_logger():
    return logging.getLogger("scianalysis")
...
def reduce_run(bluesky_run, process, protocols, output_dir):
    ...
    results_dict = ResultsDB(source_dir=output_dir).extract_single(
        infile, verbosity=verbosity
    )
...
def setup_scianalysis():
    ...
    calibration.set_beam_position(
        797 + xshift, 594.5 + yshift
    )  # OpenWAXS ~240mm from sample in PTA setup
    ...
    protocols = [
        ...
        Protocols.circular_average_q2I_fit(
            name="circular_average_q2I_fit_AE_Al3Sc",
            ylog=False,
            qn_power=1.0,
            plot_range=[1.0, 3.0, 0, None],
            fit_range=[q_Al3Sc - dq_Al3Sc, q_Al3Sc + dq_Al3Sc],
            dezing=True,
        ),
    ]
    return process, protocols, output_dir
...
def analysis(ref):
    ...
    if not run.start.get("PTA") and not (
        run.start.get("md") and run.start.get("md").get("PTA")
    ):
        ...
        process, protocols, output_dir = setup_scianalysis()
        ########################################
